...
**安全考虑**：
> "Users who can write `~/.codex/sandbox-extra.sb` already have full access to their own machine — they could just run commands outside the sandbox. User-provided rules only *relax* the sandbox (Seatbelt rules are additive on top of `(deny default)`)."
