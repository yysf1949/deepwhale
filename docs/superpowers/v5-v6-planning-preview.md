...
- v5.0 implementation is now ACTIVE: D-87 AuditLog seed is the first v5.0 evidence fixture; D-88 wired it into runToolLoop (3 emit points, optional parameter); D-89 added PersistingAuditLog (file-backed JSONL + partial-line recovery, mirror D-78 + D-80); D-90 added readAuditLog (standalone async reader). v5.0 observability+auditability quartet complete: seed + integration + persistence + query. 5 红线 scope was intentionally modified once (D-88 additive) and is back to empty from D-89 onward.
