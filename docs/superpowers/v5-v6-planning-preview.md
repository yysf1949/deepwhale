...
- v5.0 implementation is now ACTIVE: 5 evidence items across 2 themes. Theme 1: observability+auditability quartet complete (D-87 seed, D-88 integration, D-89 persistence, D-90 query). Theme 2: plugin governance seed (D-91 ToolCapability + helper + type guard + optional Tool field). 5 红线 scope was intentionally modified once (D-88 additive) and is back to empty from D-89 onward.
