...
describe('registry (D-31.4.4)', () => {
  it('media profile exposes only media tools', () => {
...
Command: `cd packages/coding-agent && pnpm exec vitest run test/unit/registry-profile-media.test.ts`
Expected: FAIL — media profile not wired yet.
...
**Step 4.5: Commit**
```bash
git add packages/coding-agent/src/tools/registry.ts packages/coding-agent/test/unit/registry-profile-media.test.ts
