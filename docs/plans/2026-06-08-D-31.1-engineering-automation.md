...
Command: `cd packages/coding-agent && pnpm exec vitest run test/unit/registry-count-23.test.ts`
Expected: FAIL — count 17 (现 1.0.15)
...
**Step 7.5: Commit**
```bash
git add packages/coding-agent/src/tools/registry.ts packages/coding-agent/test/unit/registry-count-23.test.ts
