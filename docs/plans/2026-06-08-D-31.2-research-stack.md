...
describe('registry (D-31.2.6)', () => {
  it('research profile exposes only research tools', () => {
    expect(createDefaultRegistry({ profile: 'research' }).size()).toBe(4);
  });
...
```bash
git add packages/coding-agent/src/tools/registry.ts packages/coding-agent/test/unit/registry-profile-research.test.ts
