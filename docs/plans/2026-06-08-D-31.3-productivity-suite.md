...
describe('registry (D-31.3.5)', () => {
  it('productivity profile exposes only productivity tools', () => {
...
Command: `cd packages/coding-agent && pnpm exec vitest run test/unit/registry-profile-productivity.test.ts`
Expected: FAIL — productivity profile not wired yet.
...
**Step 5.5: Commit**
```bash
git add packages/coding-agent/src/tools/registry.ts packages/coding-agent/test/unit/registry-profile-productivity.test.ts
