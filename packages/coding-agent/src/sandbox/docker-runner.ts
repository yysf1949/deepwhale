...
export class DockerSandboxRunner implements SandboxRunner {
  ...
  buildDockerArgs(
    ...
    const args: string[] = [
      ...
      '--network', this.network, // 默认 none 禁网
      '-v', `${workspaceAbs}:${CONTAINER_WORKDIR}:rw`, // 显式 workspace bind mount
      '-w', CONTAINER_WORKDIR, // 容器内工作目录
      '--tmpfs', '/tmp:size=64m,noexec,nosuid', // 临时目录禁执行
  ...
  async cleanup(): Promise<void> {
    try {
      // 找所有 deepwhale.sandbox=true 标签的容器并删
      const { stdout } = await execFileP(
        'docker',
        ['ps', '-aq', '--filter', 'label=deepwhale.sandbox=true'],
      ...
      const results = await Promise.allSettled(
        ids.map((id) => execFileP('docker', ['rm', '-f', id], { encoding: 'buffer', maxBuffer: 1024 * 1024 })),
      );
      ...
      console.warn(
        `[sandbox] cleanup error: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
}
