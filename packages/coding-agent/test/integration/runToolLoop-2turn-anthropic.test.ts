/**
 * Sprint 1c-revive-2-B-3 — runToolLoop mode layer 端到端真接 跨 Anthropic 协议路径 (1c-revive 拆分, 2026-06-04)
 *
 * 目的: 1c-revive-2-B-1 (bddd5ff) 实施 AnthropicClient tools 1c.5 拍板 + 1c-revive-2-B-2 (3fbced7)
 * 5 mock fetch 单测验证. 1c-revive-2-B-3 验 **mode layer 完整 path 跨 Anthropic 协议**: 走 runToolLoop +
 * createDefaultRegistry() + AnthropicClient (走 DeepSeek shim /anthropic 端点), test code **不**模拟
 * tool execution — 由 mode layer 内部 BashTool 真执行.
 *
 * 跟 1c-revive-2-A 差异:
 *   - 1c-revive-2-A = DeepSeekClient (OAI 协议) 走 createDefaultRegistry() mode layer 端到端
 *   - **1c-revive-2-B-3 = AnthropicClient (1d.5-B D1 broken, 实走 OAI flash 路由) 走 mode layer 端到端**
 *   - 1c-revive-2-B-3 跟 1d.5-B 一样: 验 shim 行为稳定但**不**是"真 Anthropic 协议"
 *
 * 跟 1c-revive-2-B-1 差异:
 *   - 1c-revive-2-B-1 = AnthropicClient tools 实施 (1c.5 拍板, 28 行 net production 改)
 *   - **1c-revive-2-B-3 = 走 runToolLoop 端到端真接 验 model/tool separation 走通** (0 行 production 改)
 *
 * 跟 1c-revive-2-B-2 差异:
 *   - 1c-revive-2-B-2 = 5 mock fetch 单测验证 schema 转换 (0 行 production 改, 0 真接)
 *   - **1c-revive-2-B-3 = INTEGRATION=1 真接 跨 Anthropic 协议路径** (0 行 production 改)
 *
 * 关键不变量 (mode layer 端到端 跨 Anthropic 协议, pi-agent 4-layer model/tool separation 拍板):
 *   - runToolLoop (mode layer, packages/coding-agent) 调 AnthropicClient (model layer, packages/llm)
 *   - model 跟 tool **干净 separation**: runToolLoop 只 import LLMClient interface, 不知道 AnthropicClient 实现细节
 *   - 2 turn × 2 sub-turn = 4 turn mode layer tool_use 端到端
 *   - 0 行 production code 改 (AnthropicClient 1c.5 拍板 1c-revive-2-B-1 已实施)
 *   - **不**mock LLM, **不**mock BashTool, 真实 mode layer path
 *   - BashTool 真执行, 走 execFile + echo builtin
 *   - F4 拍板 (1d.5-A.5 揭示): 跨 Anthropic 协议路径 cached>0 → cost_turn absent (跟 1c-revive-2-B Option A 4 turn 一致)
 *   - cost 公式层 7 + 层 8 软断言 cost absent, 不反算 cost (跟 1c-revive-2-A cost present 路径分叉)
 *
 * 触发条件 (跟 1c-revive-1 / 1c-revive-2-A 一致):
 ...
 *   - 任一非空 → canRun
 *
 * 关键不变量 (跟 1c-revive-2-A 镜, 但跨 Anthropic 协议):
 *   - tool_calls 触发 + tool_call_id echo (跨 Anthropic OAI flash routing 验)
 *   - BashTool 真执行 (success=true) — 跨 Anthropic 协议 routing 验
 *   - 2 turn × 2 sub-turn = 4 turn mode layer 端到端 跑通
 *   - F4 拍板跨 cached 路径全验证 (tokens_uncached = prompt - cached)
 *   - **F4 absent 跨 Anthropic 协议路径** (cost_turn / cost_currency absent 跨 4 turn, 跟 1c-revive-2-B Option A 一致)
 *   - 1d.5-B D1 mis-label stable: server 路由仍到 deepseek-v4-flash (client label claude-sonnet-4-5)
 *
 * 1c-revive 拆分 cluster 状态:
 *   - ✅ 1c-revive-1 (2d245a3): DeepSeek OAI 2 turn tool_use 端到端 (client-only, 0 行 production 改)
 *   - ✅ 1c-revive-2-A (83f87d7): DeepSeek OAI runToolLoop 端到端 (mode layer + 6 tools + BashTool, 0 行 production 改)
 *   - ✅ 1c-revive-2-B-1 (bddd5ff): AnthropicClient tools 实施 (1c.5 拍板, 28 行 net production 改)
 *   - ✅ 1c-revive-2-B-2 (3fbced7): 5 mock fetch 单测验证 schema 转换 (0 行 production 改)
 *   - 🔄 1c-revive-2-B-3 (本 test): 写 runToolLoop 端到端 跨 Anthropic 协议路径 真接 (0 行 production 改)
 *
 * 不验证 (留后续 1c-revive-2-C/3):
 *   - 跨包 session module 集成 (留 1c-revive-2-C/3)
...
// ---- 红线门: 跟 1c-revive-1 / 1c-revive-2-A / 1d.5 等真接 test 一致 ----
...
// ---- test scenario: 跟 1c-revive-2-A 镜 (简单 echo/node 算术) ----
...
describe('coding-agent mode layer — 1c-revive-2-B-3 runToolLoop 端到端 跨 Anthropic 协议路径 (1c-revive 拆分, pi-agent 4-layer)', () => {
  ...
  it(`${TURN_COUNT} × runToolLoop mode layer 跨 Anthropic 协议: 1 turn tool_call (BashTool 真执行) + 1 turn final answer`, async () => {
    // 1c.5 拍板 (1c-revive-2-B-1) 让 AnthropicClient 跟 DeepSeekClient 同 LLMClient 契约
    // → runToolLoop 干净 separation 走通 (mode layer 不知道 model layer 协议细节)
    ...
    for (let i = 0; i < TURN_COUNT; i++) {
      ...
      // 走 AnthropicClient + 6 tool registry + BashTool 真执行
      ...
      const _finalAssistantStep = assistantSteps[assistantSteps.length - 1]!;
      ...
      // ---- 收集 turn snapshot ----
      // 跟 1c-revive-2-A 镜: 取**最后**一个 tool step (LLM 收敛后), 不是**第一个** tool step
      // (LLM 自由选 bash args, 某些 args 会被 BashTool 拒绝 → content="". 不软断言"第一个
      // tool step 一定有结果", 跟 1c-revive-2-A 一样软断言 "LLM 至少跑过 1 次成功".
      // 1c-revive-2-B-3 跨 Anthropic 协议 + 真实 LLM 自由选: 失败 tool step 概率更高 (LLM 随机)).
    ...
    // 提前 dump 真实数据
    dumpSnapshots('1c-revive-2-B-3 [BEFORE assertions]', snaps);
    for (let i = 0; i < allRuns.length; i++) {
      dumpSteps(
        `1c-revive-2-B-3 [BEFORE assertions run ${i + 1}]`,
        allRuns[i]!.result.steps,
        {
          final: {
    ...
    // ---- 断言层 6: F4 拍板不变量跨 2 turn 全验证 (cached 路径) ----
    ...
    // ---- 断言层 7: F4 absent 跨 Anthropic 协议路径 (1c-revive-2-B-3 关键贡献) ----
    // F4 拍板 (1d.5-A.5 揭示): 跨 Anthropic 协议 cached > 0 → cost_turn / cost_currency absent
    // 1c-revive-2-B-3 验证这个 absent 跨 4 turn 都生效 (跟 1c-revive-2-A DeepSeek OAI cost present
    // 形成对照, 揭示 Architecture 协议分叉)
    // 跟 1c-revive-2-A 成本层 7 关键差异: 软断言 cost absent, 不反算 cost
    for (let i = 0; i < TURN_COUNT; i++) {
      const toolCallTurn = snaps[i * 2]!.usage!;
      const finalTurn = snaps[i * 2 + 1]!.usage!;
      ...
      for (const usage of [toolCallTurn, finalTurn]) {
        ...
        // 软断言: cached = 0 时 cost 仍可 absent (1b.5 保守策略 + parseAnthropicUsage 路径)
        // 不强制 cost present (跟 1c-revive-2-A OAI 路径 cost present 路径分叉)
      }
    }

    // ---- 断言层 8: 总 cost 软断言 (跨 Anthropic 协议, cost absent) ----
    // 跟 1c-revive-2-A 关键差异: 跨 Anthropic 协议 cost absent, 不反算 cost.
    // 软断言: cached 字段都 >= 0 (4 turn 都验过, 跨 cached 路径)
    let totalCached = 0;
    for (const snap of snaps) {
      if (typeof snap.usage?.cached_tokens === 'number') totalCached += snap.usage.cached_tokens;
    }
    // 4 turn totalCached 期望 > 0 (累积 messages 触发 prefix cache)
    expect(totalCached).toBeGreaterThan(0);

    // ---- 断言层 9: 揭示真实 mode layer 跨 Anthropic 协议 行为 (R-G2 风格) ----
    // 重点观测:
    //   - model 实际 label (1d.5-B mis-label stable 验: client label claude-sonnet-4-5, server 路由 deepseek-v4-flash)
    //   - cached_tokens 跨 turn 累积 pattern
    //   - **cost absent 跨 4 turn** (1c-revive-2-B-3 关键贡献, 跟 1c-revive-2-A cost present 对照)
    //   - tool_call_id 跨协议 echo (assistant.tool_calls[0].id === tool.tool_call_id)
