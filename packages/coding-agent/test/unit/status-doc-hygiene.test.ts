...
describe('status documentation hygiene (D-56)', () => {
  ...
  it('keeps the v1-v4 evidence scorecard machine-readable and caveated', () => {
    ...
    expect(scorecard.aggregatePercent).toBe(61);
    ...
    expect(scorecard.nextActions.join('\n')).not.toMatch(/^D68:/m);
    expect(scorecard.nextActions.join('\n')).not.toMatch(/^D69:/m);
    expect(scorecardMd).toContain('Aggregate evidence-backed progress: 61%');
  ...
  it('keeps the current sprint and next-work pointers aligned after D74', () => {
    for (const path of DOCS) {
      ...
      expect(block).toContain('Current sprint: D82 v2.5 investigate-goal scenario fixture');
      ...
      expect(block).toContain('Gate-1.5 live task ledger: docs/superpowers/gate-1.5-live-browser-tasks.json');
      expect(block).toContain('Next implementation slice: D83 v5 implementation kickoff (gated on user direction; v5 plan doc exists at docs/superpowers/plans/2026-06-10-d75-taskgraph-goal-recording.md)');
      expect(block).toContain('v5/v6 planning preview: docs/superpowers/v5-v6-planning-preview.json');
      expect(block).not.toMatch(/Current sprint: D81/i);
      expect(block).not.toMatch(/Next implementation slice: D82/i);
      expect(block).not.toMatch(/Current sprint: D80/i);
      expect(block).not.toMatch(/Next implementation slice: D81/i);
