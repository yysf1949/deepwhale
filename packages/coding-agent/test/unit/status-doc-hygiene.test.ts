...
describe('status documentation hygiene (D-56)', () => {
  ...
  it('keeps the current sprint and next-work pointers aligned after D74', () => {
    for (const path of DOCS) {
      ...
      expect(block).toContain('Current sprint: D97 v5.0 production hardening 2nd evidence (installSignalHandlers)');
      ...
      expect(block).toContain('D97 v5.0 production hardening 2nd evidence (installSignalHandlers)');
      ...
      expect(block).toContain('Gate-1.5 live task ledger: docs/superpowers/gate-1.5-live-browser-tasks.json');
      expect(block).toContain('Next implementation slice: D98 v5.0 production hardening 3rd evidence OR 2nd cycle (gated on user direction; uncaught exception hook OR graceful shutdown sequence OR process-level exit policy)');
      expect(block).toContain('v5/v6 planning preview: docs/superpowers/v5-v6-planning-preview.json');
      expect(block).not.toMatch(/Current sprint: D96/i);
      expect(block).not.toMatch(/Next implementation slice: D97/i);
