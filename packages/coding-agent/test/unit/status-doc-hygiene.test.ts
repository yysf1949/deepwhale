...
describe('status documentation hygiene (D-56)', () => {
  ...
  it('keeps the current sprint and next-work pointers aligned after D74', () => {
    for (const path of DOCS) {
      ...
      expect(block).toContain('Current sprint: D106 v6.0 master plan (buildV6Plan)');
      ...
      expect(block).toContain('D106 v6.0 master plan (buildV6Plan)');
      ...
      expect(block).toContain('Gate-1.5 live task ledger: docs/superpowers/gate-1.5-live-browser-tasks.json');
      expect(block).toContain('Next implementation slice: D107 v6.0 multi-agent safety seed implementation OR v5.0 4th theme cross-bridge (gated on user direction; sub-agent type + sub-agent policy + sub-agent rollback OR observability+auditability cross-bridge via PolicySnapshot)');
      expect(block).toContain('v5/v6 planning preview: docs/superpowers/v5-v6-planning-preview.json');
      expect(block).not.toMatch(/Current sprint: D105/i);
      expect(block).not.toMatch(/Next implementation slice: D106/i);
