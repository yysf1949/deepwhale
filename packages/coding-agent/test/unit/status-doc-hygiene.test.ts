...
describe('status documentation hygiene (D-56)', () => {
  ...
  it('keeps the current sprint and next-work pointers aligned after D74', () => {
    for (const path of DOCS) {
      ...
      expect(block).toContain('Current sprint: D107 v6.0 multi-agent safety seed (registerSubAgent)');
      ...
      expect(block).toContain('D107 v6.0 multi-agent safety seed (registerSubAgent)');
      ...
      expect(block).toContain('Gate-1.5 live task ledger: docs/superpowers/gate-1.5-live-browser-tasks.json');
      expect(block).toContain('Next implementation slice: D108 v6.0 multi-agent safety seed part 2 OR v5.0 4th theme cross-bridge (gated on user direction; enforceSubAgentPolicy reusing D-103 OR observability+auditability cross-bridge via PolicySnapshot)');
      expect(block).toContain('v5/v6 planning preview: docs/superpowers/v5-v6-planning-preview.json');
      expect(block).not.toMatch(/Current sprint: D106/i);
      expect(block).not.toMatch(/Next implementation slice: D107/i);
