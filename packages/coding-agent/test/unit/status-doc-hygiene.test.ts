...
describe('status documentation hygiene (D-56)', () => {
  ...
  it('keeps the current sprint and next-work pointers aligned after D74', () => {
    for (const path of DOCS) {
      ...
      expect(block).toContain('Current sprint: D96 v5.0 production hardening 1st evidence (fatal-error formatter)');
      ...
      expect(block).toContain('D96 v5.0 production hardening 1st evidence (fatal-error formatter)');
      ...
      expect(block).toContain('Gate-1.5 live task ledger: docs/superpowers/gate-1.5-live-browser-tasks.json');
      expect(block).toContain('Next implementation slice: D97 v5.0 production hardening 2nd evidence (gated on user direction; SIGINT/SIGTERM handler OR uncaught exception hook OR graceful shutdown sequence)');
      expect(block).toContain('v5/v6 planning preview: docs/superpowers/v5-v6-planning-preview.json');
      expect(block).not.toMatch(/Current sprint: D95/i);
      expect(block).not.toMatch(/Next implementation slice: D96/i);
