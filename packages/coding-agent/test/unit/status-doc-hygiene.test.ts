...
describe('status documentation hygiene (D-56)', () => {
  ...
  it('keeps the current sprint and next-work pointers aligned after D74', () => {
    for (const path of DOCS) {
      ...
      expect(block).toContain('Current sprint: D105 v5.0 cross-theme bridge (buildPolicySnapshot)');
      ...
      expect(block).toContain('D105 v5.0 cross-theme bridge (buildPolicySnapshot)');
      ...
      expect(block).toContain('Gate-1.5 live task ledger: docs/superpowers/gate-1.5-live-browser-tasks.json');
      expect(block).toContain('Next implementation slice: D106 v5.0 4th theme cross-bridge OR v6.0 master plan doc (gated on user direction; observability+auditability cross-bridge via PolicySnapshot OR multi-agent safety policy minimal seed)');
      expect(block).toContain('v5/v6 planning preview: docs/superpowers/v5-v6-planning-preview.json');
      expect(block).not.toMatch(/Current sprint: D104/i);
      expect(block).not.toMatch(/Next implementation slice: D105/i);
