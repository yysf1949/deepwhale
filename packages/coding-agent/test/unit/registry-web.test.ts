...
describe('registry web profile (legacy opt-in)', () => {
  it('includes the legacy web tools only when explicitly selected', async () => {
    const registry = await createRegistryForProfile({ profile: 'web' });
  ...
  it('does not include core coding tools in the web profile', async () => {
    const registry = await createRegistryForProfile({ profile: 'web' });
