...
> **当前分支状态（2026-06-09, `feature/d33-integration` 集成审查）**：D-33 v1.0 → v4.0 + D-33.7/D-35 集成审查中，**D-34 mock runner 不入集成**（Gate-2 LIVE 需要真 LLM client + 30-50 tool-call task，当前无真 runner）。已集成 stages：Stage 0 stabilization / v1.0 closure / v1.5 code-intel+capability-registry / v2.0 observe（memory / semantic / browser / mcp / gate-1.5）/ v2.5 plan (task-dag + planner) / v3.0 execute review (Reviewer + Compaction hook + Computer Use compat + Gate-2 35-step mock) / v4.0 research and Agent OS (Researcher + TaskGraph + Persistent Memory + Channels opt-in) / D-33.7 tool-loop policy 集成 (Reviewer + TaskGraph post-hook) / D-35 cross-session memory 集成。**未集成 follow-ups**: D-33.5.1 browser enhance (user 排除) / D-33.6.4 desktop shell (user 排除) / D-34 gate-2 mock (无真实 runner 跳过)。默认 registry 仍只暴露 coding + Code Intel essentials (19 tools)。所有新能力 opt-in (browser / mcp / channel / memory / researcher / reviewer / taskgraph)。5 红线 0 改贯穿全部 stage。
