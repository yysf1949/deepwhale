...
> **当前分支状态（2026-06-09, `feature/d33-integration` 集成审查）**：D-33 v1.0 → v4.0 + D-33.7/D-35 集成审查中，**D-34 mock runner 不入集成**（Gate-2 LIVE 需要真 LLM client + 30-50 tool-call task，当前无真 runner, PR #13 stays OPEN but unintegrated）。已集成 stages（按 release/v2.0 base 160 → 190 test files, +30 new test files）：Stage 0 stabilization（3 commit, +2 test）/ v1.0 closure（7 commit, +13 test）/ v1.5 code-intel + capability-registry（6 commit, +10 test）/ v2.0 observe（8 commit, +5 test，含 capability 3-way merge 修复）/ v2.5 plan（5 commit, +6 test）/ v3.0 execute review（6 commit, +5 test，含 Reviewer + Compaction hook + Computer Use compat + Gate-2 mock harness）/ v4.0 research and Agent OS（7 commit, +5 test）/ D-33.7 tool-loop policy 集成（1 commit, +4 test）/ D-35 cross-session memory 集成（2 commit, +3 test）。**未集成 follow-ups**: D-33.5.1 browser enhance（user 排除）/ D-33.6.4 desktop shell（user 排除）/ D-34 gate-2 mock（PR #13 OPEN but unintegrated, needs real LLM client + 30-50 tool-call task before Gate-2 LIVE claim is honest）。默认 registry 仍只暴露 coding + Code Intel essentials（19 tools）。所有新能力 opt-in（browser / mcp / channel / memory / researcher / reviewer / taskgraph）。5 红线 0 改贯穿全部 stage。**Stage 3 集成 1 conflict 修复**: capability-registry Stage 2 (class-based) vs Stage 3 (functional) 冲突,3-way merge 解决:restore Stage 2 class API + 加 'mcp'/'browser'/'computer'/'channel' profile strings + re-export Capability type。
